...
public class Skull extends Enemy {
    ...
    public Skull(Handler handler, int x, int y, int width, int height) {
    ...
    public Skull(WritableImage texture, int x, int y) {
        super(texture, x, y);
        initTexture();
    }

    public Skull(Handler handler, WritableImage texture, int x, int y, int width, int height) {
    ...
    private void initTexture() {
        for (int i = 1; i <= 3; ++i) movingDown.add(Utilities.loadWI("skull_down_" + i));
        for (int i = 1; i <= 3; ++i) movingLeft.add(Utilities.loadWI("skull_left_" + i));
        for (int i = 1; i <= 3; ++i) movingRight.add(Utilities.loadWI("skull_right_" + i));
        for (int i = 1; i <= 3; ++i) movingUp.add(Utilities.loadWI("skull_up_" + i));
